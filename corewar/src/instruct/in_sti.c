...
/*   Updated: 2017/06/06 15:16:48 by folkowic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			in_sti(t_process *proc)
{
	...
	vm_replace_int(g_env.from +
			((args->arg2 + args->arg3) % IDX_MOD), args->arg1);
	vm_color_area(g_env.from +
			((args->arg2 + args->arg3) % IDX_MOD), 4, proc->player);
	nc_refresh_color(g_env.from + ((args->arg2 + args->arg3) % IDX_MOD), 4);
	nc_lst_place_blk(&g_env.blink_st, g_env.from +
			((args->arg2 + args->arg3) % IDX_MOD));
}
